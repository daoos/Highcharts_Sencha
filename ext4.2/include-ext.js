 ...
 * 查询页面中的对应元素
 * @param name 要要查询的属性名
 * @param queryString 要查询的属性对应的字符串
 * @author 刘守权
 * @version 1.0 
 */
...
//////////////////////////////////////////
    var scriptEls = document.getElementsByTagName('script'),
       ...
       while (i--) {
        ...
        if (src.indexOf('include-ext.js') !== -1) {
            ...
            themePath = defaultPathQueryString.substring(0, defaultPathQueryString.lastIndexOf('/'));
            framePath = themePath.substring(0, themePath.lastIndexOf('/'));
    ...
    if (includeCSS) {
        document.write('<link rel="stylesheet" type="text/css" href="' + themePath + '/resources/css/ext-all' + suffix + '-debug.css"/>');
    }

    /*添加对应的JS样式JS文件*/
    document.write('<script type="text/javascript" src="' + themePath + '/ext-all-debug-w-comments' + (rtl ? '-rtl' : '') + '.js"></script>');
    /*添加对应的JS配置文件*/
    document.write('<script type="text/javascript" src="' + framePath + '/config/config.js"></script>');
    document.write('<script type="text/javascript" src="' + framePath + '/ext4.2/gantt/gnt-all-debug.js"></script>');
    document.write('<script type="text/javascript" src="' + framePath + '/ext4.2/sch/sch-all-debug.js"></script>');
    document.write('<script type="text/javascript" src="' + themePath + '/local/ext-lang-zh_CN.js"></script>');
     /*对IE6兼容性提示*/
 if(theme){
    /*添加对应的样式文件*/
        themePath = (repoDevMode ? themePath + '/..' : themePath) +'/resources/ext-theme-'+theme+'/ext-theme-'+theme+'-all.css'
        if (repoDevMode &&  window.ActiveXObject) {
            Ext = {
                _beforereadyhandler: function() {
                    Ext.Loader.loadScript({ url: themePath });
                }
            };
        } else {
            document.write('<script type="text/javascript" src="' + themePath + '" defer></script>');
        }
    }
